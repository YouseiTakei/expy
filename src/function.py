...
class F:
    ...
    def __call__(self, var=None, num=None, rqs=[1]*5):
        ...
        for obj in self.objs:
            obj.latex(cp=self.is_cp, nb=self.is_nb, rqs=rqs)
    ...
    def tex(self, var=None, num=None, rqs=[1]*5):
        for obj in self.objs:
            obj.latex(cp=True, nb=True, rqs=rqs)
        pass
        # TODO
    ...
    def get(self, var=None, num=None, ans=False, err=False):
        ...
        # if not var and len(self.objs) == 1:
        #     return vals[-1]
        return vals

    # output sub process in ipython
    def get_var(self, var=None):
        return self.objs[-1].vars[var] if var else self.objs[-1].ans
    ...
    def df(self, var=None, cap='', index=None, name='', text=False, ans=True, err=False, nb=True):
        ...
        if var:
            dict.update({
                '測定結果$%s$' % (r'/\mathrm{%s}' % ans.rm if ans.rm else''): [
                    [' $%s$' % to_sf(v) for v in arr] for arr in self.get_var_arr(var)
                ]
            })
        dict.update({
            '$%s%s%s$' % (name, ans.var, r'/\mathrm{%s}' % ans.rm if ans.rm else''): dollar(vals)
        })
        # md
        if text:
            md(text) if type(text) == type('') else md(r'全項目の計算結果を下表にまとめた.単位は $\mathrm{%s}$ である.'%self.get_var().rm)
        if nb:
