...
(
    al_, be_, ga_, de_, ep_, ze_, et_, th_, io_, ka_, la_, mu_, nu_, xi_, pi_, rh_, si_, ta_, up_,  ph_, ch_, ps_, om_, pa_
...
def math():
    return [
        ss, sub, sup, sub_0, sub_1, sub_2, sub_i, sub_k, sub_m, sub_n,
        _dash, Delta_, delta_, partial_, bar_, frac_, diff_, part_, para_]


def sub(var, any):
    return ss(r'%s_{%s}' % (sym.latex(var),  sym.latex(any)))


def sup(var, any):
    return ss(r'%s^{%s}' % (sym.latex(var),  sym.latex(any)))
...
# {%s}にスペース開けるとエラー
def Delta_(var):
    return ss(r'\Delta{%s}' % sym.latex(var))


def delta_(var):
    return ss(r'd%s' % sym.latex(var))


def partial_(var):
    return ss(r'\partial{%s}' % sym.latex(var))


def bar_(var):
    return ss(r'\bar{%s}' % sym.latex(var))


def frac_(up, down):
    return ss(r'\frac{%s}{%s}' % (Delta_(up),  Delta_(down)))


def diff_(up, down):
    return ss(r'\frac{d%s}{d%s}' % (sym.latex(de_), sym.latex(up)))


def part_(up, down):
    return ss(r'\frac{%s}{%s}' % (partial_(pa_), partial_(up)))


def para_(var, any):
    return ss(r'%s(%s)' % (var,  any))
