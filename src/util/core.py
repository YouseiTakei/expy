...
def rslt_ans(mean, uncrt=None, rm=None, dig=None, dollar=False):
    ...
    if not uncrt:
        return '%s%s' % (to_sf(mean, sf), r'\mathrm{%s}' % rm if rm else '')
    # 以降はuncrtに桁を合わせる
    dig_u = int(-num_dig(uncrt))
...
# DataFrameの数値をto_sf する
def df_to_sf(data, dig=3):
    df = data.copy()
...
def rslt_polyfit_plot(x, y1, graph=None, point='v--', line=0.75, back=False, plot=True, input=True):
    ...
    if plot:
        if graph:
            return [fig.plot(x, y, point, lw=line) for y in [y1, y2]]
        else:
            return [plt.plot(x, y, point, lw=line) for y in [y1, y2]]
    if back:
        return a, b
