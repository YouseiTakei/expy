...
class Df():
    ...
    def __call__(self, tiny=False):
        if tiny:display(Latex(r'\tiny'))
        if self.caption:display(Caption(self.caption))
        if self.dig:display(df_to_sf(self.get(), dig=self.dig))
        else   :display(self.get())
        if tiny:display(Latex(r'\normalsize'))
    ### get --------------------------------------------------------------------
    def get(self):
        df = self.df.copy()
        if self.columns: df = self.get_drop(df)
        if self.columns: df = self.get_rename(df)
        return df.rename(index={i:"" for i in list(df.index)},inplace=False)
    ### sub process --------------------
    def get_drop  (self,df): return df.drop  (columns=[i   for i,v in self.columns.items() if v==None], inplace=False)
    def get_rename(self,df): return df.rename(columns={i:v for i,v in self.columns.items() if v!=None}, inplace=False)
    ### set --------------------------------------------------------------------
...
class Fig():
    ...
    def __call__(self, is_tiny=False):
        if self.filepath: display( Image(self.filepath) )
        if self.caption : display( Caption(self.caption) )
    ### get --------------------------------------------------------------------
    def get(self):
        return self.df
    ...
    ### set --------------------------------------------------------------------
    def set(self, data=None, sheet='Sheet1', dig=None, col=None, cap=None):
        if data is not None: self.set_data(data, sheet, dig, col, cap)
        if cap: self.set_cap(cap)
    ### sub process --------------------
    def set_data(self, data, sheet, dig, col, cap):    ### データの読み取りとバックアップ
        if type(data) is pd.core.frame.DataFrame: self.df=Df(data,sheet,dig,col,cap)
        if type(data) is type(Df()): self.df = data
        if type(data) is str  :self.filepath = data
    def set_cap(self, caption):self.caption = caption
    ### ------------------------------------------------------------------------
